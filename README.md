...
```powershell
@"
## Integrar el Frontend sin desorden (Submódulo Git)

Para traer el repositorio del frontend `TFGSistemaDeGestionDeCitasParaPeluquerias` sin mezclar miles de archivos en la raíz, la opción más limpia es añadirlo como submódulo dentro de `frontend/`.

Estructura final esperada:

```
frontend/
  TFGSistemaDeGestionCitasParaPeluquerias/  (código del frontend, como submódulo)
```

### 1) Añadir el submódulo (PowerShell)

```powershell
# Desde la raíz del repo de la API
git submodule add https://github.com/alexgarzasss10/TFGSistemaDeGestionDeCitasParaPeluquerias frontend/TFGSistemaDeGestionCitasParaPeluquerias
git commit -m "Add frontend as git submodule"
```

Esto creará el directorio `frontend/TFGSistemaDeGestionCitasParaPeluquerias` y el archivo `.gitmodules` con la referencia.

### 2) Clonar/actualizar submódulos

Cuando alguien clona este repo por primera vez, debe inicializar y actualizar submódulos:
...
# Traer contenido de submódulos
git submodule update --init --recursive
```

Para actualizar el frontend a la última `main` del submódulo:

```powershell
Push-Location frontend/TFGSistemaDeGestionCitasParaPeluquerias
git checkout main
git pull
Pop-Location

# Registrar el cambio de referencia en el repo principal
git add frontend/TFGSistemaDeGestionCitasParaPeluquerias
git commit -m "Update frontend submodule"
```

Para eliminar el submódulo si hiciera falta:

```powershell
git submodule deinit -f frontend/TFGSistemaDeGestionCitasParaPeluquerias
git rm -f frontend/TFGSistemaDeGestionCitasParaPeluquerias
Remove-Item -Recurse -Force .git/modules/frontend/TFGSistemaDeGestionCitasParaPeluquerias 2>$null
git commit -m "Remove frontend submodule"
```

### 3) ¿Docker Compose?

- Si el frontend es una app de escritorio/.NET (se ve `.sln` en el repo), no hace falta integrarlo en Docker. Mantenerlo como submódulo es suficiente y ordenado.
- Si en el futuro el frontend fuera web (Vite/React, Angular, etc.), se puede añadir un `docker-compose.override.yml` con un servicio `frontend` que sirva en `http://localhost:5173` y comunicarlo con la API en `http://localhost:8000`. Ahora mismo no se incluye para evitar ruido innecesario.
